...
release: tag release-deps 
	gox -os="linux" -os="windows" -arch="amd64" -arch="386" -ldflags "-X main.version=${VERSION}" -output="build/{{.Dir}}_{{.OS}}_{{.Arch}}" .
	gox -os="darwin" -arch="amd64" -arch="arm64" -ldflags "-X main.version=${VERSION}" -output="build/{{.Dir}}_{{.OS}}_{{.Arch}}" .
